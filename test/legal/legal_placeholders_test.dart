...
import 'package:orbits_flutter/pages/settings/legal_offer_page.dart';
...
void main() {
  test('lawyer brief lists technical facts and the six open items', () {
    final brief = File('docs/legal-requirements-for-lawyer.md').readAsStringSync();
  ...
  testWidgets('offer screen shows pending placeholder, not invented counsel text',
      (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        theme: testOrbitsTheme(),
        home: const LegalOfferPage(),
      ),
    );

    expect(find.byKey(kLegalOfferBodyKey), findsOneWidget);
    expect(find.text(kLegalPendingPlaceholder), findsWidgets);
    expect(find.textContaining('серверах Владельца'), findsNothing);
  });
  ...
  testWidgets('onboarding agreement step has an 18+ checkbox and blocks finish',
      (tester) async {
    var finished = false;
    await tester.pumpWidget(
      MaterialApp(
    ...
    await tester.pump();
    expect(finished, isFalse, reason: 'finish must stay disabled without 18+');
  });
}
