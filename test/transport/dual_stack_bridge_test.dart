...
void main() {
  ...
  final chatScope = attachmentConversationScope(
  ...
  test(
    'DualStackBridge persists ciphertext onto the carrier journal',
    () async {
      ...
      expect(revoked.any((row) => row['kind'] == 'deviceRevoked'), isTrue);
  ...
  test(
    'contactBlocked and attachmentExpired journal without secrets',
    () async {
      final (a, b, _) = await linked();
  ...
  test(
    'journalContactBlocked refuses URL-shaped peerId before journal',
    () async {
      final (a, b, _) = await linked();
      a.journalContactBlocked(peerId: 'https://evil', blocked: true);
      expect(
        a.journal.records.any(
  ...
  test(
    'journalAttachmentExpired refuses URL-shaped eventId or conversationId',
    () async {
      final (a, b, _) = await linked();
  ...
  test('attachment chunks journal ciphertext and never the fileKey', () async {
    ...
    expect(rows.any((row) => row['kind'] == 'attachmentPublished'), isTrue);
  ...
  test(
    'attach-chunk is not Drop and decrypts with the local fileKey',
    () async {
      final (a, b, _) = await linked();
      final drop = <Object>[];
      ...
      expect(drop, isEmpty);
      expect(
        jsonEncode(a.journal.records.map((r) => r.fields).toList()),
  ...
  test('attach-chunk nested fileKey is refused and does not decrypt', () async {
    ...
    final clean = ResumableAttachment.chunk(
      plain,
      key,
      scope: chatScope,
    ...
    transport.emitEvent(
      TransportFrame(
        ...
        jsonPayload({
          ...
          'fileId': 'chat-clean-chunk',
          'hash': clean.hash,
          'b64': base64Encode(clean.ciphertext),
          'index': 0,
          'offset': 0,
          'totalBytes': plain.length,
        }),
  ...
  test(
    'inbound attach-chunk with URL fileId does not decrypt or buffer',
    () async {
      final (a, b, _) = await linked();
      final drop = <Object>[];
      b.onDrop = (peer, packet) => drop.add(packet);
  ...
  test(
    ...
    () {
      final src = File(
        'lib/transport/dual_stack_bridge.dart',
      ).readAsStringSync();
  ...
  test(
    'attach-chunk via ciphertext path sendFile is not Dart frameB64',
    () async {
      final (a, b, _) = await linked();
      final drop = <Object>[];
      b.onDrop = (peer, packet) => drop.add(packet);
      ...
      expect(
        await a.sendAttachmentCipherPath(
          ...
          TransportFileDescriptor(
            ...
            fileId: 'chat-file-path',
            plaintextBytes: write.plaintextBytes,
          ),
      ...
      expect(
        a.journal.records.any(
          (r) => r.kind == ReplicationEventKind.attachmentPublished,
        ),
        isTrue,
      );
      expect(
      ...
      expect(
        File('lib/transport/dual_stack_bridge.dart').readAsStringSync(),
        contains('openCipherPathToPlaintext'),
      );
      expect(
        File('lib/transport/dual_stack_bridge.dart').readAsStringSync(),
  ...
  test(
    'decryptInboundAttachmentPath writes plaintext to a path and consumes',
    () async {
      final (a, b, _) = await linked();
      final key = List<int>.generate(32, (i) => i + 8);
  ...
  test(
    'sendAttachmentCipherPath with URL fileId throws and delivers no attach-chunk',
    () async {
      final (a, b, _) = await linked();
      final drop = <Object>[];
      b.onDrop = (peer, packet) => drop.add(packet);
      ...
      expect(error is StateError || ok == false, isTrue);
      await Future<void>.delayed(const Duration(milliseconds: 40));
      expect(
        await b.decryptInboundAttachment(
          'ORBIT-AAAAAAAAAAAAAAAA',
  ...
  test('receiver rejects a replayed attach-chunk nonce', () async {
    final (a, b, _) = await linked();
    final key = List<int>.generate(32, (i) => i + 4);
    final plain = List<int>.generate(90, (i) => i + 1);
    final chunk = ResumableAttachment.chunk(
  ...
  test(
    'loopback resume after partial delivery reconstructs the file',
    () async {
      final (a, b, _) = await linked();
      ...
      expect(got, Uint8List.fromList(plain));
      final transport = a.transport as LoopbackOrbitsTransport;
      final frames = transport.sentAttachmentFrames
          .where(
            (f) =>
                f['type'] == 'attach-chunk' && f['fileId'] == 'resume-partial',
          )
          .toList();
      expect(frames.length, greaterThanOrEqualTo(2));
      final encoded = jsonEncode(frames);
      expect(encoded, isNot(contains('fileKey')));
      expect(encoded, isNot(contains(base64Encode(key))));
      expect(encoded, isNot(contains(base64Encode(plain))));
      expect(
        jsonEncode(a.journal.records.map((r) => r.fields).toList()),
        isNot(contains('fileKey')),
      );
      expect(
        jsonEncode(a.journal.records.map((r) => r.fields).toList()),
        isNot(contains(base64Encode(key))),
      );
      await a.detach();
      await b.detach();
    },
  ...
  test(
    'sendRoomPacket refuses nested fileKey and still sends host-plaintext',
    () async {
      final (a, b, packets) = await linked();
      kRoomPlaintextSessionAck.setAcknowledged(true);
      final rooms = <Map<String, Object?>>[];
      ...
      expect(
        rooms.any(
          (r) => r['id'] == 'm-secret' || jsonEncode(r).contains('fileKey'),
        ),
        isFalse,
      );
      expect(a.rooms.state.messages.any((m) => m['id'] == 'm-secret'), isFalse);
      expect(b.rooms.state.messages.any((m) => m['id'] == 'm-secret'), isFalse);
      ...
      expect(b.rooms.state.messages.any((m) => m['text'] == 'hello'), isTrue);
      kRoomPlaintextSessionAck.reset();
      await a.detach();
      await b.detach();
    },
  ...
  test(
    'sendRoomPacket refuses URL-shaped roomId before apply and send',
    () async {
      final (a, b, _) = await linked();
      kRoomPlaintextSessionAck.setAcknowledged(true);
      final rooms = <Map<String, Object?>>[];
      ...
      await Future<void>.delayed(const Duration(milliseconds: 20));
      expect(
        a.rooms.state.members.containsKey('ORBIT-CCCCCCCCCCCCCCCC'),
        isFalse,
      );
      expect(
        b.rooms.state.members.containsKey('ORBIT-CCCCCCCCCCCCCCCC'),
        isFalse,
      );
  ...
  test(
    'sendCallSignal refuses empty or URL-shaped callId before transport.send',
    () async {
      final (a, b, _) = await linked();
      CallSignal? seen;
  ...
  test(
    'native path attachment survives loss and resumes from offset',
    () async {
      setHyperswarmRollout(HyperswarmRollout.internal);
      final pair = loopbackPair();
      final secrets = DiscoverySecretStore()
      ...
      expect(
        File(received['path'] as String).readAsBytesSync(),
        src.readAsBytesSync(),
      );
      await a.detach();
      await b.detach();
  ...
  test('HTTP storage peer rejects plaintext and anonymous writes', () async {
    ...
    expect(await post('/v1/grant', {'token': '', 'quotaBytes': 10}), 400);
    expect(await post('/v1/grant', {'token': 'cap', 'plaintext': 'hi'}), 400);
    expect(
  ...
  test('lost messages and journal mismatch rollback to PeerJS', () async {
    ...
    expect(nativeRollbackLog.single.reason, NativeRollbackReason.messagesLost);
    setHyperswarmRollout(HyperswarmRollout.internal);
    a.hypercore.append(
      const JournalRecord(
        seq: 99,
  ...
  test(
    'Autobase writers converge on the native carrier without Hypercore plaintext',
    () async {
      final (a, b, _) = await linked();
      expect(
  ...
  test(
    ...
    () {
      ...
      expect(src, isNot(contains("import 'dart:io'")));
      final apply = src
          .split('void _applyRoom')[1]
          .split('Future<void> sendCallSignal')[0];
  ...
  test(
    ...
    () {
      ...
      expect(revoke.indexOf('://'), lessThan(revoke.indexOf('journal.append')));
      final authorize = src
  ...
  test(
    ...
    () async {
      ...
      a.journal.append(ReplicationEventKind.messageEnvelopeCreated, {
        'eventId': 'hydrate-env',
        'conversationId': 'ORBIT-BBBBBBBBBBBBBBBB',
      ...
      expect(
        restored.hypercore.blocks.every(
          (r) => !r.fields.containsKey('plaintext'),
        ),
        isTrue,
      );
      expect(kRoomsApplicationE2eImplemented, isFalse);
  ...
  test(
    ...
    () async {
      ...
      a.journal.append(ReplicationEventKind.roomMembershipChanged, {
        'roomId': 'room-nested',
        'peerId': 'ORBIT-CCCCCCCCCCCCCCCC',
        'action': 'join',
        'displayName': 'C',
  ...
  test(
    ...
    () async {
      ...
      expect(
        restored.journal.records.every(
          (r) => !r.fields.containsKey('plaintext'),
        ),
        isTrue,
      );
      ...
      expect(
        restored.hypercore.blocks.every(
          (r) => !r.fields.containsKey('fileKey'),
        ),
        isTrue,
      );
      expect(
  ...
  test(
    'room_file_chunk projects Autobase attachment metadata without b64',
    () async {
      final (a, b, _) = await linked();
      kRoomPlaintextSessionAck.setAcknowledged(true);
      expect(
  ...
  test('dial fans out to recipient device transports', () async {
    ...
    expect(
      ...
      containsAll(['ORBIT-BBBBBBBBBBBBBBBB', 'ORBIT-B1B1B1B1B1B1B1B1']),
    );
    expect(
  ...
  test(
    'attach maps known contacts without sending discovery secrets',
    () async {
      setHyperswarmRollout(HyperswarmRollout.internal);
      final pair = loopbackPair();
  ...
  test(
    'authorizeDevice remembers transportPeerId Noise without a secret',
    () async {
      setHyperswarmRollout(HyperswarmRollout.internal);
      final pair = loopbackPair();
      await pair.$1.start(
      ...
      await Future<void>.delayed(const Duration(milliseconds: 20));
      expect(
        pair.$1.rememberedPeers.any(
          (p) => p.peerId == 'ORBIT-B1B1B1B1B1B1B1B1',
        ),
        isTrue,
      );
      expect(
  ...
  test(
    'own-device tablet is a sendTarget and dials on the local secret',
    () async {
      final devices = DeviceRegistry();
      ...
      expect(
        ...
        containsAll(['ORBIT-BBBBBBBBBBBBBBBB', 'ORBIT-A2A2A2A2A2A2A2A2']),
      );
      expect(a.discoverySecretFor('ORBIT-A2A2A2A2A2A2A2A2'), localSecret);
      expect(
  ...
  test(
    'sendEncrypted source-scan uses sendTargets not only transportTargets',
    () {
      final src = File(
        'lib/transport/dual_stack_bridge.dart',
      ).readAsStringSync();
  ...
  test(
    'loopback Autobase lists DualStack membership after room_join',
    () async {
      final (a, _, _) = await linked();
      expect(
        a.sendRoomPacket('ORBIT-BBBBBBBBBBBBBBBB', {
  ...
  test('ack maps use the receipt channel; pathChanged is recorded', () async {
    ...
    (a.transport as LoopbackOrbitsTransport).emitEvent(
      const TransportPathChanged('ORBIT-BBBBBBBBBBBBBBBB', TransportPath.relay),
    );
    await Future<void>.delayed(const Duration(milliseconds: 20));
  ...
  test(
    'signed device binding authenticates; unsigned is disconnected',
    () async {
      final (a, b, _) = await linked();
  ...
  test('revoked device and TOFU mismatch fail connect checks', () async {
    ...
    await Future<void>.delayed(const Duration(milliseconds: 80));
    expect(right.canUseNative('ORBIT-AAAAAAAAAAAAAAAA'), isFalse);
    expect(right.bindingFailures['ORBIT-AAAAAAAAAAAAAAAA'], 'deviceNotRevoked');
    await right.detach();
    await pair.$1.stop();
    await pair.$2.stop();
    ...
    expect(c.bindingFailures['ORBIT-BBBBBBBBBBBBBBBB'], 'tofuDoesNotConflict');
    await c.detach();
    await d.detach();
  });

  test(
    'noise mismatch fails before signature when a connection key is set',
    () async {
  ...
  test(
    'sendEncrypted waits for DeviceBinding auth before native chat',
    () async {
      setHyperswarmRollout(HyperswarmRollout.internal);
      final pair = loopbackPair();
      final secrets = DiscoverySecretStore()
  ...
  test(
    'native calls, drop, and inbound frames wait for DeviceBinding auth',
    () async {
      setHyperswarmRollout(HyperswarmRollout.internal);
      final pair = loopbackPair();
      final secrets = DiscoverySecretStore()
