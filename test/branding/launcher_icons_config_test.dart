...
// (Android + Windows) launcher-icon generation in CI. A regression here (wrong
// source path, Android/adaptive/Windows generation turned off, or web flipped
...
void main() {
  ...
  setUpAll(() {
    ...
    cfg = (loadYaml(file.readAsStringSync())
        as YamlMap)['flutter_launcher_icons'] as YamlMap;
  });

  test('single source of truth exists and is a non-empty PNG', () {
    const src = 'tool/branding/orbits_icon_source.png';
    expect(cfg['image_path'], src,
  ...
  test('Android legacy + adaptive icons are configured', () {
    ...
    expect(cfg['adaptive_icon_background'], isNotNull,
    ...
    expect(File(fg as String).existsSync(), isTrue,
