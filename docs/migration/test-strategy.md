...
| 4 | `dual_stack_bridge_test` two natives, `v2:` / wireHello; `preferredWorkletBackend`; worklet Noise→ORBIT map; ConnectionsNotifier `_nativeCarrierFor`; `rollbackNativeToPeerjs` on start fail / worklet-exit / journal / mailbox / lost messages / battery / relay blow-up | Two physical devices |
