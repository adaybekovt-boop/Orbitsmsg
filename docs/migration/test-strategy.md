...
| 9 | 10 and 50 MiB chunk/resume; `chunkStream` / `chunkFromByteStream`; native `sendFileFromPath`; path-backed receive + `harness-file-resume` after loss; PeerJS Drop `sendFileRanged` + `PathDropChunkStore` (default live path); chat/room/profile `withData: kIsWeb` + `readPickedBytes`; native chat XOR-to-temp + `sendFile` `attach-chunk` when `canUseNative`; `attachmentPublished` journal (no `fileKey`) | Web picker still supplies bytes; default chat/room still PeerJS b64 |
