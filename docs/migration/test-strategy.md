...
| 9 | 10 and 50 MiB chunk/resume; `chunkFromByteStream`; native `sendFileFromPath`; path-backed receive + `harness-file-resume` after loss; PeerJS Drop `sendFileRanged` + `PathDropChunkStore` (default live path); chat/room `withData: kIsWeb` + `readPickedBytes` | Web picker still supplies bytes; chat/room still PeerJS b64 |
