...
| 5 | Identity-signed caps on native connect and as a PeerJS `wireHello.caps` sibling; native `DeviceBinding` is identity-signed over `signedPayload` (not the capability-record signature, not Noise) and advertises mailbox/hypercore/multi-device wire names; contact QR may carry discovery secret `d=`; secrets persist vault-wrapped | Physical pair not run |
