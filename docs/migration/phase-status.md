...
| 4 | App boot binds native host when rollout ≠ off; prefers Hyperswarm **only** with module + explicit HyperDHT bootstrap (`ORBITS_DHT_BOOTSTRAP` / directory rows), else loopback; worklet `connect` maps Noise public key → ORBIT contact id (never `HASH(peerId)`); Noise seed ≠ identity; ConnectionsNotifier waits for that carrier (and DeviceBinding auth) before native chat/files/calls/rooms/Drop; loopback natives exchange `v2:` / wireHello | Default still PeerJS; two physical natives not run |
