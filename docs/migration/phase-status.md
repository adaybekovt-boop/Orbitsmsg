...
| 9 | Product path is `orbits-file-v1` / `FileTransferCoordinator`; worklet `sendFile` is harness-only (`harness-file-*`). Incoming lookup covers canonical + legacy layouts. Native inbound persists a path/sha256 descriptor, not the blob bytes | `test/attachments/incoming_paths_test.dart`; `test/storage/db_secure_storage_test.dart`; `tool/connectivity_harness/test/echo_file.test.js` (10 MiB + 50 MiB) | PeerJS Drop / room attachments still copy some blobs; 10–50 MiB on two devices **open** |
| 10 | Distinct persisted transport/writer keys + signed local binding. DualStack fans out through `DeviceRatchetSessions` when sessions are bound and revoke drops those sessions; QR `acceptDeviceLink` still does not mint a ratchet pair by itself | `test/devices/local_device_material_test.dart`; `test/devices/dual_stack_device_ratchet_test.dart` | Live multi-device hardware **open** |
| 11–12 | RoomManager records host-plaintext Autobase membership/channel/message events; writers converge in `RoomAutobaseLog`. Room packets still go DualStack when `canUseNative`. Warning and `kRoomsApplicationE2eImplemented` stay false. Desktop native plugins stay OTP1 fail-closed (`BARE_RUNTIME_MISSING`); debug desktop uses `LocalWorkletPlatform` | `test/rooms/autobase_and_epoch_test.dart`; `test/peer/room_manager_test.dart` | Live rooms / signed desktop OTP1 **open** |
