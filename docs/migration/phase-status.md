...
| 8 | Blind mailbox + HTTP `StoragePeerClient` + local loopback fleet (3/2/2; HyperDHT bootstrap when the module is present, else HTTP marked `protocol: http`) + HyperDHT `relayThrough` keys on extra testnet relay rows + opaque wake HTTP intake; mailbox deposit enqueues an opaque wake (no peer id / body) through `dispatchMailboxWake` using on-device APNs/FCM tokens when present (never a dummy `undeployed` token) and optional loopback `ORBITS_PUSH_GATEWAY_ORIGIN`; `PushSender` still **refuses** APNs/FCM send; APNs/FCM request builders stay opaque; APNs provider **ES256 JWT** is built (not identity-signing-v1) and still not sent; FCM service-account **RS256 JWT** is built (not identity-signing-v1); FCM OAuth JWT-bearer token **request** is built and still not exchanged/sent; FCM send Bearer is an OAuth `access_token` (never the assertion JWT); Android `DEVICE_IDLE` → Doze; drain tombstones ciphertext; block list before mailbox decrypt/persist; backlog rollback; local HTTP mailbox 256 KiB body cap + 32 deposits/10s per token + retention GC; unsigned lab directories with dummy timestamps still load; epoch-millis `expiresAt` is enforced; `buildApnsSendHttp` / `dispatchApnsSendHttp` describe the Apple HTTPS POST *shape* (injected `post`; not HTTP/2 HPACK) and `PushSender.sendApns` does not call them while the flag is false | No deployed public fleet / APNs/FCM send / live signed directory |
| 9 | Drop packets on native `attachment` channel; 10–50 MiB resume tests; path-streamed native `sendFileFromPath`; receiver writes chunks at offset to a path (never a growing Dart `Uint8List`); `harness-file-resume` handshake so an interrupted send continues from the contiguous offset; PeerJS Drop `sendFileRanged` / `PathDropChunkStore` so the default live path also streams from disk; Drop UI never `readAsBytes` of the picked file on native; web Drop uses picker `readStream` (DartSha256 incremental hash on `file-end`, no full-file `Uint8List`); inbound native `attach-chunk` is written to a ciphertext path (loopback/Bare) and decrypted from that path (no Dart `b64` ingest on the receive path); chat/room/profile native pickers use `withData: kIsWeb` and `readPickedBytes` (stat then read under the cap) **unless** `canUseNative`, in which case 1:1 chat materializes a local path (`materializePickedLocalPath` — reuse picker path or stream/bytes onto a temp file, `withReadStream: !kIsWeb`) then XORs to a temp ciphertext path and Bare/loopback `sendFile` emits `attach-chunk` (fileKey only in the ratchet envelope; no Dart `frameB64` chunks over IPC); DualStackBridge journals `attachmentPublished` ciphertext (never `fileKey`); native 1:1 decrypts inbound ciphertext **to a plaintext path** (`decryptInboundAttachmentPath` / `xorCipherPathToPlaintextFile`) and persists that path in Drift `file_blobs.data` via `saveFileBlobFromPath` (empty wrapped `bytes`, never a 50 MiB plaintext blob; cap [kMaxNativeAttachBytes]); outbound `sendFileFromPath` also copies into Application Support `orbits-file-blobs` (`persistLocalAttachmentPath`) so `/tmp` cleanup does not drop the FileTile path; outbox retry reuses the pending `fileKeyB64` (`nativeAttachFileKeyFromPayload`) so a second XOR key cannot be minted over the same `fileId`; native room picker streams host-plaintext `room_file_chunk` from a path (64 KiB, cap still 12 MiB, no fileKey) and materializes pathless picks the same way (`materializePickedLocalPath`); Drop pathless native picks also materialize then `sendFileFromPath` (no Dart `Uint8List` send); native `sendVoice` / byte `sendFile` materialize to a temp path (`writeBytesToTempPath`) then `sendChatAttachmentFromPath` (no wire `b64` when `canUseNative`); inbound DualStack `attach-chunk` writes ciphertext at offset (`openInboundCipherPath` / `writeInboundCipherChunk`) and only falls back to `_inboundAttach` memory on web / path-write failure; inbound chunked voice assembles via `assembleNativeAttachment` without persisting `fileKeyB64`; web/PeerJS room send stays `room_msg` b64 | Default chat/room send is still PeerJS; rooms stay host-plaintext bytes |
| 10 | Device-link QR + revoke journal events + per-identity fan-out + three-device RatchetState isolation test; QR keys from Noise seed (not dummy bytes); native `dial` passes Noise public key and fans out to `transportTargets` (recipient devices; unpublished extras are swallowed); `openReliable` still calls that `dial` so chat mount reaches every registered device, not only the conversation id; revoke drops that device's transport ratchet only; chat / ephemeral / Drop / path+chunk attachments / call signals / room+Autobase control fan out via `sendTargets`; Hypercore envelopes replicate to send-targets and device/block events to every authenticated peer **and** own-device `sendTargets`; own-device transports use the local advertise secret and are dialed after `rememberKnownPeers` / `authorizeDevice`; loopback `connect` joins the remote discovery topic even when the local advertise topic differs | No live multi-device ratchet sessions on hardware |
| 11–12 | Room maps on native carrier; live `room_join` / `room_msg` / `room_file_chunk` (metadata only, no b64) project into Autobase; worklet `AutobaseProjection` (`autobase.js`) converges the same events over control frames (not Holepunch Autobase npm; not in `bare_stdlib.zip`); Autobase writers converge over DualStackBridge (`autobase-event`); membership is journaled without message plaintext; DualStackBridge attach rebuilds Autobase members from that journal after restart; loopback `hydrateAutobase` / `listAutobase` apply the same membership snapshot as the worklet; `InProcessBareWorklet` applies hydrate rows instead of an empty stub and, on a shared hub, routes `connect` / `send` / path `sendFile` (refuses `://` / `bytes` / `fileKey`) as IPC events; room voice uses a DualStack `call` mesh (`rv-` callId + `media.channel=room-voice`, glare by peer-id order, 1:1 overlay ignores those signals) for each `canUseNative` member that also advertised `room-voice-v1` (old DualStack clients without that bit stay on PeerJS room-voice so they do not treat `rv-` as a 1:1 ring) even on default-live; PeerJS `callPeer` remains for members that are not on the native carrier or lack `room-voice-v1` (and is skipped entirely under isolation) | Live rooms still PeerJS host-plaintext |
...
- APNs / FCM: local opaque wake HTTP + `PushGateway` intake + `PushSender`
  ...
  POST *shape* the same way (injected `post`; **not** HTTP/2 HPACK).
  `PushSender.sendApns` does not call them while the flag is false.
