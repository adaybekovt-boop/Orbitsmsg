...
- APNs / FCM: local opaque wake HTTP + `PushGateway` intake + `PushSender`
  ...
  identity key) and is still not sent. An RS256 FCM service-account JWT
  may be built from Google PKCS#8 PEM (not identity-signing-v1). The
  OAuth JWT-bearer POST to `oauth2.googleapis.com/token` may be built
  and is still not exchanged. FCM send stays off.
