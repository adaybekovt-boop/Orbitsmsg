...
| 3 | Official Holepunch `bare-runtime` **1.31.0** is pinned and fetched at **build time**; Linux/Windows hosts spawn that verified CLI with the bundled worklet; Android/iOS use official BareKit when the XCFramework/AAR is packaged and otherwise stay `BARE_RUNTIME_MISSING`. Release still refuses Node. | `tool/bare/fetch-official-runtime.sh` + `verify-runtime.sh`; harness Bare/Corestore/DHT tests; host `g++` fail-closed tests | Apple/Authenticode signing of the Holepunch binary **open**; BareKit `prebuilds.zip` not pinned into mobile CI by default (396MB) |
