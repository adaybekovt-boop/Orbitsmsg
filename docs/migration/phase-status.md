...
| 3 | Official Holepunch `bare-runtime` **1.31.0** is pinned and fetched at **build time**; Linux/Windows hosts spawn that verified CLI with the bundled worklet; Android/iOS use official BareKit `Worklet` / `BareWorklet` when a locally fetched, sha-pinned `classes.jar` + `jni` or `BareKit.xcframework` is present and otherwise stay `BARE_RUNTIME_MISSING`. Release still refuses Node. | `tool/bare/fetch-official-runtime.sh` + `verify-runtime.sh`; `tool/bare/verify-kit-hooks.sh` (no 396MB download); harness Bare/Corestore/DHT/two-runtime tests; host `g++` fail-closed tests; Linux/Windows bundle presence checks | Apple/Authenticode signing of the Holepunch binary **open**; packaging BareKit into store binaries requires `bash tool/bare/fetch-official-runtime.sh --kit` on a cached, sha-pinned job |
