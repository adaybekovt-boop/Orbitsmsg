...
| 10 | Distinct persisted transport/writer keys + signed local binding. After a native admit the dialer offers a per-device ratchet handshake; DualStack fans out through those sessions and revoke drops them. QR `acceptDeviceLink` authorizes the device and does not put private ratchet material in the QR | `test/devices/local_device_material_test.dart`; `test/devices/dual_stack_device_ratchet_test.dart` | Live multi-device hardware **open** |
