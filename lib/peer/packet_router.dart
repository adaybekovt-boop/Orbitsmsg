...
/// True iff [frame] is something the ENCRYPTED RELAY fallback is allowed to
/// deliver inbound:
///   • a wire-ciphertext String (Double-Ratchet `v2:` envelope), or
///   • a plaintext handshake-control Map (`wireHello` / `wireRekey` — public
///     key material only).
/// Everything else — `room_*` control, drop/file-transfer frames, binary
/// chunks ([Uint8List]), arbitrary plaintext maps (`{type:'msg'/'text'}`),
/// malformed data — is NOT relay-safe and must be dropped. The relay is a dumb
/// router for encrypted TEXT + handshake only; it must NEVER reach the
/// drop/room/file subsystems or be treated as a trusted plaintext message.
bool isRelaySafeFrame(Object? frame) {
  if (frame is String) return isWireCiphertext(frame);
  if (frame is Map) {
    final t = frame['type'];
    return t == 'wireHello' || t == 'wireRekey';
  }
  return false;
}

/// Build a handler for RELAY-delivered frames. Unlike [createPacketHandler]
/// (the DataChannel handler, which also fast-paths binary/file/room/drop), this
/// accepts ONLY relay-safe frames ([isRelaySafeFrame]) and routes them through
/// the SAME [dispatchReliableInbound] decrypt/verify path — so there is no
/// second, weaker crypto path AND no way for a relay (or a peer abusing it) to
/// inject file/room/drop/binary traffic. Non-relay-safe frames are dropped via
/// [onDropped] and NEVER produce an ack.
PacketHandler createRelayPacketHandler(
  String remoteId,
  PacketRouterCtx ctx, {
  void Function(Object? frame)? onDropped,
}) {
  return (data) async {
    if (!isRelaySafeFrame(data)) {
      // room_* / drop / file-* / Uint8List / plaintext msg / malformed.
      onDropped?.call(data);
      return;
    }
    // Same path as a DataChannel reliable frame: handshake accept or
    // ratchet-decrypt → dispatchReliablePlaintext (ack gated on saveMessage).
    final handled = await dispatchReliableInbound(
      data,
      ctx.conn,
      remoteId,
